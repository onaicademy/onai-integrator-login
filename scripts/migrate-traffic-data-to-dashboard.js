/**
 * Скрипт миграции данных из Landing DB в Traffic Dashboard DB
 * Мигрирует таблицы: landing_leads → leads, journey_stages, 
 * traffic_stats → traffic_sales_stats, express_course_sales → traffic_sales,
 * main_product_sales → traffic_sales, challenge3d_sales → challenge3d_sales
 */
...
const TRAFFIC_DB = {
  ...
  key: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9ldG9kYWV4bmpjdW5rbGtkbGt2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjYyMTc2OTMsImV4cCI6MjA4MTc5MzY5M30.isG3OnecdTr7nKecQGtCxQIRCZcrdiZggvKa7DaFtjg'
};
...
/**
 * Трансформация данных для таблицы landing_leads → leads
 */
function transformLead(lead) {
  return {
    ...
    payment_method: lead.amocrm_lead_id ? 'amocrm' : null,
...
function transformTrafficStat(stat) {
  return {
    ...
    average_order_value: 0, // Calculate if needed
...
function transformExpressSale(sale) {
  return {
    ...
    lead_id: null, // Will be mapped later
...
function transformChallenge3DSale(sale) {
  return {
    ...
    payment_method: 'card', // Default, can be updated
    lead_id: null, // Will be mapped later
    prepayment_deal_id: null, // Will be mapped later
    sale_date: sale.sale_date,
    created_at: sale.created_at,
    updated_at: sale.updated_at
  };
 ...
 */
async function main() {
  console.log('🚀 Начало миграции данных из Landing DB в Traffic Dashboard DB\n');

  const results = [];

  // 1. Миграция landing_leads → leads (фильтрация существующих)
  console.log('\n🔍 Получение списка существующих lead_id из leads...');
  const { data: existingLeads } = await trafficClient
    .from('leads')
    .select('id');
  
  const existingLeadIds = new Set(existingLeads?.map(l => l.id) || []);
  console.log(`📋 Найдено существующих lead_id: ${existingLeadIds.size}`);
  ...
  results.push({ table: 'landing_leads → leads', ...leadsResult });
  ...
  // 2. Миграция journey_stages (только для существующих lead_id)
  console.log('\n🔍 Получение списка существующих lead_id из leads...');
  const { data: existingLeadsForStages } = await trafficClient
    .from('leads')
    .select('id');
  
  const existingLeadIdsForStages = new Set(existingLeadsForStages?.map(l => l.id) || []);
  console.log(`📋 Найдено существующих lead_id: ${existingLeadIdsForStages.size}`);
  ...
  const stagesResult = await migrateTable(
    ...
    (stage) => existingLeadIdsForStages.has(stage.lead_id)
  );
  results.push({ table: 'journey_stages', ...stagesResult });
