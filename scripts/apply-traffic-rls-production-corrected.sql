...
USING (
  auth.jwt() ->> 'role' = 'targetologist' AND
  traffic_fb_campaigns.team_name = auth.jwt() ->> 'team'
);
...
USING (
  auth.jwt() ->> 'role' = 'targetologist' AND
  traffic_sales_stats.team_name = auth.jwt() ->> 'team'
);
...
USING (
  auth.jwt() ->> 'role' = 'targetologist' AND
  traffic_user_sessions.team_name = auth.jwt() ->> 'team'
);
...
USING (
  ...
  traffic_onboarding_progress.user_id IN (
    SELECT id FROM public.traffic_users WHERE team_name = auth.jwt() ->> 'team'
  )
);
...
USING (
  auth.jwt() ->> 'role' = 'targetologist' AND
  lead_tracking.team_name = auth.jwt() ->> 'team'
);
...
USING (
  auth.jwt() ->> 'role' = 'targetologist' AND
  sales_activity_log.team_name = auth.jwt() ->> 'team'
);
...
USING (
  ...
  traffic_fb_ads.fb_campaign_id IN (
    SELECT id FROM public.traffic_fb_campaigns WHERE team_name = auth.jwt() ->> 'team'
  )
);
...
USING (
  auth.jwt() ->> 'role' = 'targetologist' AND
  all_sales_tracking.team_name = auth.jwt() ->> 'team'
);
