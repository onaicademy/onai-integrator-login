...
// Tripwire
const tripwireUrl = process.env.TRIPWIRE_SUPABASE_URL || 'https://pjmvxecykysfrzppdcto.supabase.co';
const tripwireKey = process.env.TRIPWIRE_SERVICE_ROLE_KEY;

if (!tripwireKey) {
  console.error('❌ TRIPWIRE_SERVICE_ROLE_KEY not found in .env');
  process.exit(1);
}

const tripwireSupabase = createClient(tripwireUrl, tripwireKey);
