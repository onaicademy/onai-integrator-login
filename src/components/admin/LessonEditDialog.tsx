...
export function LessonEditDialog({ open, onClose, onSave, lesson, moduleId, onVideoUploaded }: LessonEditDialogProps) {
  ...
  const loadLessonData = async (lessonId: number) => {
    ...
    try {
      ...
      } else if (videoRes?.video?.video_url) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        if (videoFile) {
          ...
          xhr.upload.addEventListener('progress', (e) => {
            if (e.lengthComputable) {
              ...
              setUploadStatus(`📹 Загружено: ${loadedMB} MB / ${totalMB} MB (${percent}%)`);
              console.log(`📊 Upload progress: ${loadedMB} MB / ${totalMB} MB (${percent}%)`);
            }
          });
      ...
      if (videoFile) {
        ...
        // ✅ ИСПОЛЬЗУЕМ XMLHttpRequest для прогресс-бара (как в Tripwire!)
        await new Promise((resolve, reject) => {
          const xhr = new XMLHttpRequest();
          ...
          xhr.upload.addEventListener('progress', (e) => {
            if (e.lengthComputable) {
              ...
              const totalMB = (e.total / (1024 * 1024)).toFixed(1);
              const percent = Math.round((e.loaded / e.total) * 100);
              const adjustedPercent = 10 + Math.round(percent * 0.4); // 10-50%
