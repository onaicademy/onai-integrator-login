...
export function TripwireLoginForm({ onForgotPassword }: TripwireLoginFormProps) {
  ...
  const getButtonStyles = (state: ButtonState) => {
    switch (state) {
      ...
      case 'success':
        return 'bg-[#00FF94] text-black';
      default:
        return 'bg-[#00FF94] hover:bg-[#00cc75] text-black hover:scale-[1.02]';
    }
  };
      ...
      >
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#00FF94] transition-colors"
            disabled={isLoading}
          >
      ...
      >
        <div className="flex items-center gap-2">
          <Checkbox
            ...
            className="border-gray-600 data-[state=checked]:bg-[#00FF94] data-[state=checked]:border-[#00FF94]"
          />
          <label
            htmlFor="remember"
        ...
        <button
          ...
          className="text-sm text-[#00FF94] hover:text-[#00cc75] transition-colors"
        >
          Забыли пароль?
        </button>
      </motion.div>
