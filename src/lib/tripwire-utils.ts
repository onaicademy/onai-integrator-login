...
/**
 * Получить время для прямого эфира (СТАТИЧНЫЙ ТЕКСТ)
 * 🔒 ЗАГЛУШКА: 17 декабря в 20:00 (динамический таймер временно отключён)
 */
export const getStreamTime = (): string => {
  // 🔒 СТАТИЧНОЕ ЗНАЧЕНИЕ (вернём динамический потом)
  return '17 декабря в 20:00';
};

/**
 * Получить обратный отсчёт до эфира (СТАТИЧНЫЙ ТЕКСТ)
 * 🔒 ЗАГЛУШКА: Ожидайте (динамический таймер временно отключён)
 */
