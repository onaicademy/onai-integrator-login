...
export default function MainPlatformDebugPanel() {
  ...
  async function loadDebugData() {
    try {
      ...
      const [statsData, errorsData, logsData] = await Promise.all([
        api.get(`/api/debug/stats?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`),
        api.get(`/api/debug/errors?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}&limit=50`),
        api.get(`/api/debug/logs?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}&limit=100${filterType !== 'ALL' ? `&eventType=${filterType}` : ''}`),
      ]);
  ...
  async function handleCleanup() {
    ...
    try {
      await api.post('/api/debug/cleanup');
  ...
  async function searchUsers() {
    ...
    try {
      const response = await api.get(`/api/debug/search-users?q=${encodeURIComponent(searchTerm)}`);
  ...
  async function loadUserLogs(userId: string) {
    ...
    try {
      const [logsResponse, statsResponse] = await Promise.all([
        api.get(`/api/debug/user-logs/${userId}?limit=100`),
        api.get(`/api/debug/user-stats/${userId}`),
      ]);
