...
const tripwireModules = [
  ...
  },
  {
    ...
    moduleNumber: 3,
    title: "МОДУЛЬ 3: Создание вирусных Reels",
...
export default function TripwireProductPage() {
  ...
  useEffect(() => {
    if (!tripwireUser?.id) return; // ✅ CRITICAL FIX: session.user has 'id', not 'user_id'
    ...
    const loadUnlocks = async () => {
      try {
        ...
        const { data: progressData, error: progressError } = await tripwireSupabase
          ...
          .eq('tripwire_user_id', tripwireUser.id) // ✅ CRITICAL FIX: Use id (auth.users.id)
    ...
    loadUnlocks();
  }, [tripwireUser?.id]); // ✅ CRITICAL FIX: Depend on id
  ...
  const handleUnlockComplete = async () => {
    if (!currentUnlock || !tripwireUser?.id) return; // ✅ CRITICAL FIX: Check id

    try {
      // Mark animation as shown
      await api.post('/api/tripwire/module-unlocks/mark-shown', {
        userId: tripwireUser.id, // ✅ CRITICAL FIX: Use id (auth.users.id)
