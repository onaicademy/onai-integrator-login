...
/**
 * 📧 ПРЯМАЯ ОТПРАВКА EMAIL ЧЕРЕЗ RESEND
 * Загружает .env, получает студентов из БД, отправляет через Resend
 */
...
async function sendEmails() {
  ...
  if (!process.env.TRIPWIRE_SUPABASE_URL) {
    console.error('❌ TRIPWIRE_SUPABASE_URL не найден в .env');
    process.exit(1);
  }
  if (!process.env.TRIPWIRE_SERVICE_ROLE_KEY) {
    console.error('❌ TRIPWIRE_SERVICE_ROLE_KEY не найден в .env');
    process.exit(1);
  }
  ...
  console.log('🔍 TRIPWIRE_SUPABASE_URL:', process.env.TRIPWIRE_SUPABASE_URL?.substring(0, 30) + '...');
  console.log('🔍 TRIPWIRE_SERVICE_ROLE_KEY:', process.env.TRIPWIRE_SERVICE_ROLE_KEY?.substring(0, 30) + '...');
  ...
  console.log('📊 Получаю список студентов из БД...');
  ...
  console.log('');
  console.log('📧 Начинаю отправку через Resend...');
  console.log('');
  ...
  if (sent === students.length) {
    ...
    console.log(`⚠️ Отправлено ${sent} из ${students.length}`);
  }
  console.log('');
}
