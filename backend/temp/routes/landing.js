...
const amocrm_js_1 = require("../lib/amocrm.js");
const scheduledNotifications_js_1 = require("../services/scheduledNotifications.js");
const facebook_conversion_js_1 = require("./facebook-conversion.js");
const mobizon_js_1 = require("../services/mobizon.js");
const telegramService_js_1 = require("../services/telegramService.js");
...
const landingSupabase = (0, supabase_js_1.createClient)(LANDING_SUPABASE_URL, LANDING_SUPABASE_SERVICE_KEY, {
    auth: {
 ...
 */
async function createAmoCRMContact(lead) {
    ...
    try {
        ...
        const response = await axios_1.default.post(`https://${AMOCRM_DOMAIN}/api/v4/contacts`, [contactData], {
            headers: {
 ...
 */
async function getUnsortedStatusId(pipelineId) {
    if (!AMOCRM_DOMAIN || !AMOCRM_ACCESS_TOKEN) {
        return null;
    }
    try {
        const response = await axios_1.default.get(`https://${AMOCRM_DOMAIN}/api/v4/leads/pipelines/${pipelineId}`, {
            headers: {
                'Authorization': `Bearer ${AMOCRM_ACCESS_TOKEN}`,
                'Content-Type': 'application/json'
            }
        ...
        if (statuses && statuses.length > 0) {
            ...
            const unsortedStatus = statuses.find((s) => s.name?.toLowerCase().includes('не разобран') ||
 ...
 */
async function createAmoCRMLead(lead, contactId) {
    if (!AMOCRM_DOMAIN || !AMOCRM_ACCESS_TOKEN) {
    ...
    try {
        ...
        const response = await axios_1.default.post(`https://${AMOCRM_DOMAIN}/api/v4/leads`, [leadData], {
            headers: {
                'Authorization': `Bearer ${AMOCRM_ACCESS_TOKEN}`,
                'Content-Type': 'application/json'
            }
...
🌐 Источник: Лендинг /expresscourse`;
                await axios_1.default.post(`https://${AMOCRM_DOMAIN}/api/v4/leads/${leadId}/notes`, [
                    {
                        note_type: 'common',
                        params: {
                            text: noteText
        ...
        (async () => {
            try {
                ...
                try {
                    ...
                    const telegramSent = await (0, telegramService_js_1.sendLeadNotification)({
                        name,
                        phone,
                ...
                const amocrmResult = await retryWithBackoff(() => (0, amocrm_js_1.createOrUpdateLead)({
                    name,
                    email: email || undefined,
                    phone,
                    utmParams: metadata?.utmParams || {},
                ...
                if (campaignSlug && facebook_conversion_js_1.PIXEL_CONFIGS[campaignSlug]) {
                    const pixelConfig = facebook_conversion_js_1.PIXEL_CONFIGS[campaignSlug];
                    ...
                    const fbResult = await retryWithBackoff(() => (0, facebook_conversion_js_1.sendConversionApiEvent)(pixelConfig, 'Lead', { email: email || '', phone, name }, referer, userAgent, ipAddress, fbc, fbp), 2, // 2 retries
...
router.get('/health', async (req, res) => {
    ...
    if (AMOCRM_DOMAIN && AMOCRM_ACCESS_TOKEN) {
        try {
            const response = await axios_1.default.get(`https://${AMOCRM_DOMAIN}/api/v4/account`, {
 ...
 */
router.post('/amocrm/callback', async (req, res) => {
    try {
...
// ============================================
router.post('/proftest', async (req, res) => {
    try {
        ...
        (async () => {
            try {
                ...
                if (campaignSlug && facebook_conversion_js_1.PIXEL_CONFIGS[campaignSlug]) {
                    ...
                    const fbc = req.cookies?._fbc;
                    const fbp = req.cookies?._fbp;
                    const fbResult = await retryWithBackoff(() => (0, facebook_conversion_js_1.sendConversionApiEvent)(pixelConfig, 'Lead', { email, phone, name }, // Email обязательный
                    referer, userAgent, ipAddress, fbc, fbp), 2, // 2 retries
                    1000 // 1s initial delay
                    );
                    if (fbResult !== null) {
                        console.log('✅ Facebook Conversion API: Lead event sent');
                ...
                (0, scheduledNotifications_js_1.scheduleProftestNotifications)({
                    name,
                    email,
                    phone,
                    leadId,
...
router.post('/resend/:leadId', async (req, res) => {
    try {
        ...
        if (needsEmail) {
            try {
                emailSuccess = await (0, scheduledNotifications_js_1.sendProftestEmailWithTracking)(lead.name, lead.email, leadId);
                if (emailSuccess) {
        ...
        if (needsSMS) {
            try {
                smsSuccess = await (0, mobizon_js_1.sendProftestResultSMS)(lead.phone, leadId);
                if (smsSuccess) {
                    await landingSupabase
                        .from('landing_leads')
    ...
    try {
        ...
        try {
            if (AMOCRM_DOMAIN && AMOCRM_ACCESS_TOKEN) {
                ...
                const response = await axios_1.default.get(`https://${AMOCRM_DOMAIN}/api/v4/leads`, {
                    params: {
        ...
        const amocrmLeadIds = new Set(amocrmLeads.map((l) => l.id.toString()));
        const missingInAmoCRM = dbLeads?.filter(l => l.amocrm_lead_id && !amocrmLeadIds.has(l.amocrm_lead_id.toString())) || [];
...
// ============================================
router.post('/sync-to-amocrm/:leadId', async (req, res) => {
    try {
        const { leadId } = req.params;
        ...
        let aggregatedUTM = lead.metadata?.utmParams || {};
        // Extract ProfTest answers from journey stages
        let proftestAnswers = lead.metadata?.proftestAnswers || lead.metadata?.answers || [];
        ...
        if (journeyStages.length > 0) {
            journeyStages.forEach((stage) => {
                ...
                if (stage.stage?.startsWith('payment_')) {
                    const method = stage.stage.replace('payment_', '');
