...
exports.PIXEL_CONFIGS = void 0;
exports.sendConversionApiEvent = sendConversionApiEvent;
const express_1 = __importDefault(require("express"));
const crypto_1 = __importDefault(require("crypto"));
const router = express_1.default.Router();
exports.PIXEL_CONFIGS = {
    muha: {
        id: 'muha',
        name: 'Muha Campaign',
...
async function sendConversionApiEvent(pixelConfig, eventName, userData, eventSourceUrl, userAgent, ipAddress, fbc, fbp) {
    try {
        // Hash user data for privacy (required by Facebook)
        const hashedEmail = crypto_1.default
            .createHash('sha256')
            .update(userData.email.toLowerCase().trim())
            .digest('hex');
        const hashedPhone = crypto_1.default
            .createHash('sha256')
            .update(userData.phone.replace(/\D/g, ''))
            .digest('hex');
        const [firstName, ...lastNameParts] = userData.name.split(' ');
        const lastName = lastNameParts.join(' ');
        const hashedFirstName = crypto_1.default
            .createHash('sha256')
        ...
        const response = await fetch(`https://graph.facebook.com/v18.0/${pixelConfig.pixelId}/events?access_token=${pixelConfig.conversionApiToken}`, {
            method: 'POST',
            headers: {
...
router.post('/track-lead', async (req, res) => {
    try {
        ...
        const pixelConfig = exports.PIXEL_CONFIGS[campaignSlug];
        if (!pixelConfig) {
            return res.status(400).json({
        ...
        const result = await sendConversionApiEvent(pixelConfig, 'Lead', { email, phone, name }, referer, userAgent, ipAddress, fbc, fbp);
        res.json({
            success: true,
