...
exports.recoverPendingNotifications = recoverPendingNotifications;
exports.sendProftestEmailWithTracking = sendProftestEmailWithTracking;
exports.scheduleProftestNotifications = scheduleProftestNotifications;
...
const mobizon_js_1 = require("./mobizon.js");
const proftest_result_email_js_1 = require("../templates/proftest-result-email.js");
...
let tripwireSupabase = null;
function getLandingSupabase() {
    if (!landingSupabase) {
        landingSupabase = (0, supabase_js_1.createClient)(process.env.LANDING_SUPABASE_URL || '', process.env.LANDING_SUPABASE_SERVICE_KEY || '');
    }
    return landingSupabase;
}
function getTripwireSupabase() {
    if (!tripwireSupabase) {
        tripwireSupabase = (0, supabase_js_1.createClient)(process.env.TRIPWIRE_SUPABASE_URL || '', process.env.TRIPWIRE_SUPABASE_SERVICE_KEY || '');
    }
 ...
 */
async function saveScheduledNotificationToDB(leadId, notification, scheduledFor) {
    try {
    ...
    try {
        ...
        for (const notif of data) {
            ...
            if (delayMs <= 0) {
                ...
                console.log(`⏰ [Lead ${notif.lead_id}] Rescheduling in ${Math.round(delayMs / 1000)}s`);
 ...
 */
async function sendProftestEmailWithTracking(name, email, leadId) {
    try {
        ...
        const htmlContent = (0, proftest_result_email_js_1.generateProftestResultEmail)(name, trackingUrl);
        // Send via Resend
        ...
        console.log(`✅ [Lead ${leadId}] Email sent successfully (ID: ${result.data?.id})`);
        return true;
    }
    catch (error) {
...
async function sendProftestSMSWithTracking(phone, email, leadId) {
    try {
        ...
        const success = await (0, mobizon_js_1.sendProftestResultSMS)(phone, leadId);
        if (!success) {
...
async function executeNotification(notification) {
    ...
    if (emailSuccess && smsSuccess) {
        ...
        await updateNotificationStatus(leadId, 'failed', errorMessages.join('; '));
 ...
 */
async function createUnifiedLead(notification) {
    try {
        const { leadId, name, email, phone, sourceCampaign } = notification;
        ...
        console.error(`❌ [Lead ${notification.leadId}] Failed to create in unified_lead_tracking:`, error.message);
    }
}
/**
 * Main scheduling function (WITH PERSISTENT DB STORAGE)
 */
function scheduleProftestNotifications(data) {
    ...
    const timeoutId = setTimeout(async () => {
        try {
            ...
            await updateNotificationStatus(leadId, 'failed', error.message);
        }
 ...
 */
async function cancelScheduledNotification(leadId) {
