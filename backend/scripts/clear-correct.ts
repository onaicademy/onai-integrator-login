...
async function clearProgress() {
  ...
  // Get tripwire_user with user_id
  const { data: tripwireUser, error: userError } = await tripwireSupabase
    .from('tripwire_users')
    .select('id, user_id')
    .eq('email', email)
    .single();
  ...
  const tripwireUserId = tripwireUser.id;  // for tripwire_progress
  const mainUserId = tripwireUser.user_id; // for module_unlocks, video_tracking, achievements
  
  console.log(`✅ Tripwire User ID: ${tripwireUserId}`);
  console.log(`✅ Main User ID: ${mainUserId}\n`);
  
  // 1. Clear video_tracking (uses main user_id)
  ...
  // 3. Clear user_achievements (uses main user_id)
  ...
  // 4. Clear module_unlocks (uses main user_id)
  ...
  // 5. Reset module 16 as unlocked (вводный модуль) - uses main user_id
