...
export function getStageByPaymentMethod(method: 'kaspi' | 'card' | 'manager'): number {
  switch (method) {
    case 'kaspi':
      return AMOCRM_CONFIG.STAGES.НАЖАЛ_КАСПИ_ОПЛАТА; // 82174958
    case 'card':
      return AMOCRM_CONFIG.STAGES.НАЖАЛ_ПРОДАМУС; // 81854578
    case 'manager':
      return AMOCRM_CONFIG.STAGES.НАЖАЛ_ЧАТ_С_МЕНЕДЖЕРОМ; // 81854582
    default:
      return AMOCRM_CONFIG.STAGES.ЗАЯВКА_С_ПРОФТЕСТА; // 81856842
  }
}

// Helper function to get initial stage for proftest leads
export function getProfTestInitialStage(): number {
  return AMOCRM_CONFIG.STAGES.ЗАЯВКА_С_ПРОФТЕСТА; // 81856842
}
