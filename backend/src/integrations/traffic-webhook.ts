...
router.post('/traffic', async (req: Request, res: Response) => {
  try {
    ...
    console.log('📋 [Traffic Webhook] Content-Type:', req.headers['content-type']);
    console.log('📋 [Traffic Webhook] Raw Body Type:', typeof req.body);
    ...
    if (!leads || (Array.isArray(leads) && leads.length === 0)) {
      ...
      console.log('📋 [Traffic Webhook] Body Keys:', bodyKeys);
      ...
      console.log('🔄 [Traffic Webhook] Parsed leads from URL-encoded:', leads);
    }

    if (!leads || (Array.isArray(leads) && leads.length === 0)) {
      console.log('⚠️ [Traffic Webhook] No leads in payload after all parsing attempts');
