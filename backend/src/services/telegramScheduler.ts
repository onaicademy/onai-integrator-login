...
// 📅 Воскресенье 20:00 - Недельный отчет
export function scheduleWeeklyReport() {
  // Каждое воскресенье в 20:00 (Asia/Almaty timezone)
  cron.schedule('0 20 * * 0', async () => {
    console.log('📅 [Воскресенье 20:00] Генерация недельного отчета...');
    try {
      const report = await generateWeeklyReport();
      await sendToAllChats(report, 'Markdown');
      console.log('✅ [Воскресенье 20:00] Недельный отчет отправлен');
    } catch (error) {
      console.error('❌ [Воскресенье 20:00] Ошибка отправки отчета:', error);
    }
  }, {
    timezone: 'Asia/Almaty'
  });
  console.log('✅ Расписание воскресенье 20:00 (недельный отчет) настроено');
}
...
export function initScheduler() {
  ...
  console.log('   • Вс 20:00 - Недельный отчет');
}
