...
interface CreateShortLinkParams {
  ...
  campaign?: string;
  source?: string;
  expiresInDays?: number; // Опционально: через сколько дней истекает ссылка
}
...
export async function createShortLink(params: CreateShortLinkParams): Promise<string | null> {
  try {
    ...
    while (attempts < maxAttempts) {
      ...
      if (!existing) break; // Код уникален
 ...
 */
export async function getShortLinkStats(shortCode: string): Promise<ShortLinkStats | null> {
  try {
    const { data, error } = await supabase
      .from('short_links')
 ...
 */
export async function getLeadShortLinksStats(leadId: string): Promise<ShortLinkStats[]> {
  try {
    const { data, error } = await supabase
      .from('short_links')
      .select('short_code, original_url, clicks_count, unique_ips, last_clicked_at, created_at')
 ...
 */
export async function deactivateShortLink(shortCode: string): Promise<boolean> {
  try {
