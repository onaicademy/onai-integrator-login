...
// Then import Supabase (needs env vars)
...
async function loadInitialAdAccounts() {
  ...
  targetologists.forEach((t: any) => console.log(`   - ${t.team} (${t.email})`));
  console.log('');

  // 3. Fetch ad accounts from Facebook
  console.log('📱 Загружаем ad accounts из Facebook API...');
  let fbAdAccounts: FBAdAccount[] = [];
  ...
  try {
    ...
    fbAdAccounts.forEach((acc: FBAdAccount) => {
  ...
  for (const targetologist of targetologists) {
    ...
    const adAccountsData = fbAdAccounts.map((acc: FBAdAccount) => ({
      id: acc.id,
      name: acc.name,
