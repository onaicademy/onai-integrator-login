...
## 🚨 **КРИТИЧЕСКИ ВАЖНО - ПЕРЕД ЛЮБЫМИ ИЗМЕНЕНИЯМИ!**

### ⛔ **НЕ ТРОГАТЬ ПУБЛИЧНЫЕ ЛЕНДИНГИ!**

**Идет платный трафик! Любая ошибка = потерянные деньги!**

**Прочитай перед любыми изменениями:**
👉 **[⚠️_CRITICAL_DO_NOT_MODIFY_LANDING.md](./⚠️_CRITICAL_DO_NOT_MODIFY_LANDING.md)**

**Что нельзя изменять:**
- ❌ ProfTest (`/integrator/proftest`)
- ❌ ExpressCourse (`/integrator/expresscourse`)
- ❌ Landing API endpoints (`/api/landing/*`)
- ❌ AmoCRM integration (`backend/src/lib/amocrm.ts`)
- ❌ Scheduled notifications (`backend/src/services/scheduledNotifications.ts`)

**Если нужно изменить** → **СПРОСИ ВЛАДЕЛЬЦА СНАЧАЛА!**
