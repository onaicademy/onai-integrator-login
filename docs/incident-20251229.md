# 🚨 Incident Report - Platform Stabilization
**Date:** 2025-12-29
...
### Success Criteria
- [ ] Student login → profile loads → lessons list loads
- [ ] Sales Manager login → tripwire admin stats/users load
- [ ] Landing lead submit → Telegram + amoCRM deal created correctly
...
- [ ] Fix payment method → pipeline stage mapping
- [ ] Add retries + idempotency
- [ ] Verify: 10 test leads → Telegram + amoCRM

---

## 🔍 GOLDEN FLOWS (Verification Checklist)
...
Internet → Nginx (Host)
    ...
    └── api.onai.academy
        ├── /api/main/* → main-backend:3000
        ├── /api/traffic/* → traffic-backend:3001
        └── /api/tripwire/* → tripwire-backend:3002
```
...
### Current Issues
1. ❌ **Build-time vs Runtime ENV**: VITE_*/NEXT_PUBLIC_* baked at build
2. ❌ **Multiple GoTrueClient**: Browser has multiple auth instances
3. ❌ **Direct Supabase calls**: Frontend calls secondary projects directly
4. ❌ **No healthchecks**: Containers "up" but services dead
5. ❌ **RLS disabled**: tripwire_users has no RLS
6. ❌ **Plaintext passwords**: generated_password stored unencrypted
...
4. **No orchestration**: Frontend → Supabase direct (bypasses server logic)
...
### PHASE 0 (Completed)
- ✅ Created hotfix/federation-stabilize branch
