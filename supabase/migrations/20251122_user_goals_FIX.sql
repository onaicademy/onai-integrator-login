...
DROP TABLE IF EXISTS weekly_goal_reports CASCADE;
DROP FUNCTION IF EXISTS update_user_goals_updated_at();
DROP FUNCTION IF EXISTS analyze_user_goals(UUID);
DROP FUNCTION IF EXISTS check_goal_achievements(UUID);
DROP FUNCTION IF EXISTS award_xp_for_goal_completion();
...
CREATE FUNCTION update_user_goals_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE FUNCTION analyze_user_goals(p_user_id UUID)
RETURNS JSONB AS $$
DECLARE
...
CREATE FUNCTION check_goal_achievements(p_user_id UUID)
RETURNS VOID AS $$
DECLARE
...
CREATE FUNCTION award_xp_for_goal_completion()
RETURNS TRIGGER AS $$
DECLARE
  v_xp_amount INTEGER;
BEGIN
