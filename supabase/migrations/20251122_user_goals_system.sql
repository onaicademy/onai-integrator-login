-- ============================================
-- СИСТЕМА ЦЕЛЕЙ С AI НАСТАВНИКОМ
-- ============================================
-- 
-- Таблицы:
-- 1. user_goals - Цели студентов
-- 2. goal_achievements - Достижения за цели
-- 3. weekly_goal_reports - Еженедельные отчеты
-- 
-- Функции:
-- 1. analyze_user_goals() - Анализ продуктивности
-- 2. award_xp_for_goal_completion() - Начисление XP
-- 3. check_goal_achievements() - Проверка достижений
-- 4. generate_weekly_goal_reports() - Еженедельные отчеты
-- ============================================

-- ============================================
-- 1. ТАБЛИЦА: user_goals
-- ============================================
...
-- ============================================
-- 5. ФУНКЦИЯ: check_goal_achievements()
-- ============================================
...
BEGIN
  ...
  LOOP
    ...
    IF v_user_count >= v_achievement.condition_value THEN
      ...
      RAISE NOTICE 'Достижение разблокировано: % (+% XP)', v_achievement.title, v_achievement.xp_reward;
    END IF;
  END LOOP;
END;
$$ LANGUAGE plpgsql;
...
-- ============================================
-- 6. ТРИГГЕР: award_xp_for_goal_completion()
-- ============================================
...
BEGIN
  IF NEW.status = 'done' AND (OLD.status IS NULL OR OLD.status != 'done') THEN
    ...
    RAISE NOTICE '✅ Цель выполнена! +% XP для пользователя %', v_xp_amount, NEW.user_id;
  END IF;
...
BEGIN
  ...
  RAISE NOTICE '   - goal_achievements (8 достижений)';
  RAISE NOTICE '   - weekly_goal_reports';
  RAISE NOTICE '';
  RAISE NOTICE '✅ Функции созданы:';
  RAISE NOTICE '   - analyze_user_goals()';
  RAISE NOTICE '   - check_goal_achievements()';
  RAISE NOTICE '   - award_xp_for_goal_completion()';
  RAISE NOTICE '';
  RAISE NOTICE '✅ Триггеры:';
  RAISE NOTICE '   - Автоматическое начисление XP при выполнении цели';
  RAISE NOTICE '   - Автоматическая разблокировка достижений';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 Система целей готова к использованию!';
  RAISE NOTICE '═══════════════════════════════════════════════════';
END $$;
